...
- Use an installed .NET SDK that can build the target frameworks listed below. This repository currently targets: **net10.0**.
- The solution is `Codebelt.Extensions.Carter.slnx`. Central package versions are maintained in `Directory.Packages.props`.
...
- `Codebelt.Extensions.Carter.slnx` is the solution used for local development.
...
```powershell
dotnet restore "Codebelt.Extensions.Carter.slnx"
dotnet build "Codebelt.Extensions.Carter.slnx" --configuration Release --no-restore
```
...
The CI test plan currently runs **6** project(s) and excludes **0** project(s). The workflow also has an optional macOS test job.
...
```powershell
dotnet pack "Codebelt.Extensions.Carter.slnx" --configuration Release --no-restore
```
...
By contributing to Extensions for Carter API by Codebelt, you agree that your contributions will be licensed under the MIT license.
